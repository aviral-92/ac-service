...
import com.customer.management.tool.extractor.UserExtractor;
...
public class UserManagementDaoImpl implements UserManagementDao {
	...
	public UserManagementDaoImpl(ResourceBundleMessageSource messageSource) {
		// super();
	...
	public String addUser(UserDetailHistory userDetail, CMTLogin login) throws Exception {
		...
		if (!isUserExist(userDetail)) {
			...
			int success = jdbcTemplate.update(QueryConstant.INSERT_IN_LOGIN, args.toArray());
			if (success > 0) {
				args = new ArrayList<>();
				...
				int temp = jdbcTemplate.update(QueryConstant.INSERT_USERDETAIL, args.toArray());
				if (temp > 0) {
					response = messageSource.getMessage(MessageCode.USER_ADDED.getValue(), null,
							Locale.getDefault());
				} else {
					response = messageSource.getMessage(MessageCode.USER_NOT_ADDED.getValue(), null,
							Locale.getDefault());
				}
			} else {
			...
			response = messageSource.getMessage(MessageCode.USER_EXIST.getValue(), null, Locale.getDefault());
		}
		LOG.info(response);
		return response;
	}
		...
		if (!StringUtils.isEmpty(detail)) {
			if (!StringUtils.isEmpty(detail.getUsername())) {
				...
				List<UserDetailHistory> detailHistories = jdbcTemplate
						.query("Select * from userdetail where username = ? ", new UserExtractor(), args);
	...
	public List<UserDetailHistory> getUsers(UserDetailHistory detail) {
		...
		if (!StringUtils.isEmpty(detail)) {
			StringBuilder query = new StringBuilder(QueryConstant.GET_USERDETAIL);
			List<String> args = new ArrayList<>();
			/*if (detail.getStatus() != null
					&& (detail.getStatus().equalsIgnoreCase(UserStatusType.ACTIVATE.getPrperty())
							|| detail.getStatus().equalsIgnoreCase(UserStatusType.DEACTIVATE.getPrperty()))) {
				args.add(detail.getStatus());
			} else {
				args.add(UserStatusType.DELETE.getPrperty());
			}*/
			...
			userDetail = jdbcTemplate.query(query.toString(), new UserExtractor(), args.toArray());
		}
		return userDetail;
	}
		...
		if (!StringUtils.isEmpty(detail)) {
			if (org.apache.commons.lang3.StringUtils.isNotEmpty(detail.getUsername())
				...
				if (!StringUtils.isEmpty(user) && !user.isEmpty() && user.get(0).getStatus() != null) {
					if (user.get(0).getStatus().equalsIgnoreCase(UserStatusType.ACTIVATE.getPrperty())) {
						StringBuilder query = new StringBuilder("select * from  customer_mgmt_tool.userdetail where username != ? ");
						List<String> args = new ArrayList<>();
						...
						if (!StringUtils.isEmpty(detail.getEmail()) && !StringUtils.isEmpty(detail.getMobile())) {
							...
							return messageSource.getMessage(MessageCode.INAPPROPRIATE.getValue(), null,
									Locale.getDefault());
						}
						...
						if (users.isEmpty()) {
							...
							if (jdbcTemplate.update(QueryConstant.UPDATE_USER, arg) > 0) {
								response = messageSource.getMessage(MessageCode.USER_UPDATED.getValue(), null,
										Locale.getDefault());
							}
						} else {
	...
	private void commonsAddUserDetailHistory(String description, UserDetailHistory userDetailHistory) {
		...
		int executed = jdbcTemplate.update(QueryConstant.INSERT_USERDETAILHISTORY, args);
		if (executed > 0) {
