	...
	public String addUser(UserDetailHistory userDetail, CMTLogin login) throws Exception {
		...
		if (!isUserExist(userDetail)) {
			...
			if (success > 0) {
				...
				if (temp > 0) {
					response = messageSource.getMessage(CMTMessageCode.USER_ADDED.getValue(), null,
							Locale.getDefault());
				} else {
					response = messageSource.getMessage(CMTMessageCode.USER_NOT_ADDED.getValue(), null,
							Locale.getDefault());
				}
			} else {
			...
			response = messageSource.getMessage(CMTMessageCode.USER_EXIST.getValue(), null, Locale.getDefault());
		}
		LOG.info(response);
		return response;
	}
				...
				if (!StringUtils.isEmpty(user) && !user.isEmpty() && user.get(0).getStatus() != null) {
					if (user.get(0).getStatus().equalsIgnoreCase(UserManagementCode.ACTIVATE.getPrperty())) {
						...
						if (!StringUtils.isEmpty(detail.getEmail()) && !StringUtils.isEmpty(detail.getMobile())) {
							...
							return messageSource.getMessage(CMTMessageCode.INAPPROPRIATE.getValue(), null,
									Locale.getDefault());
						}
						...
						if (users.isEmpty()) {
							...
							if (jdbcTemplate.update(CMTQueryConstant.UPDATE_USER, arg) > 0) {
								response = messageSource.getMessage(CMTMessageCode.USER_UPDATED.getValue(), null,
										Locale.getDefault());
							}
						} else {
